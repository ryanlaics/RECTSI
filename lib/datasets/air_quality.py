...
class AirQuality(PandasDataset):
    ...
    def load(self, impute_nans=True, small=False, masked_sensors=None):
        ...
        time_in_day = np.tile(time_ind, [1, 36, 1]).transpose((2, 1, 0))
        feature_list.append(time_in_day)
        dow = df.index.dayofweek
        dow_tiled = np.tile(dow, [1, 36, 1]).transpose((2, 1, 0))
        feature_list.append(dow_tiled)
        date = np.concatenate(feature_list, axis=-1)
        new_df_1 = pd.DataFrame(date[..., 0], index=df.index)
        new_df_2 = pd.DataFrame(date[..., 1], index=df.index)
        ...
        return final_df, dist, mask, adjs, positions, adj_label
